function fetchProject() {
  $.ajax({
                ...
                <div class="col-md-9">
                    <div class="card" style="border: none;">
                        <div id="headingOne" class="card-header bg-white shadow-sm border-0">
                            <h6 class="mb-0 font-weight-bold"><a href="#" data-toggle="collapse" data-target="#collapse${
                              project.id
                            }" aria-expanded="true" aria-controls="collapseOne" class="d-block position-relative text-uppercase collapsible-link py-2">${
          project.name
        }</a>
                        ...
                        <div id="collapse${
                          project.id
                        }" aria-labelledby="headingOne" data-parent="#accordionExample" class="collapse">
                            ...
                            <div class="card-body px-4">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <button type="button" data-id="${
                                      project.id
                                    }" class="btn btn-sm btn-primary invite-project"><i class="fa fa-user-plus"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    .fail((err) => {
      toastr.error(
        Array.isArray(err.responseJSON.msg)
          ? err.responseJSON.msg.join("<br />")
    ...
    .done((response) => {
      ...
      for (let todo of response) {
        ...
        if (todo.status) {
          ...
          !todo.status &&
          Date.parse(todo.due_date) < new Date(Date.now() - 86400000)
        ) {
        ...
        $("#todo-container").append(`
                ...
                <div class="col-md-9">
                    <div class="card" style="border: none;">
                        <div id="headingOne" class="card-header bg-white shadow-sm border-0">
                            <h6 class="mb-0 font-weight-bold"><a href="#" data-toggle="collapse" data-target="#collapse${
                              todo.id
                            }" aria-expanded="false" aria-controls="collapseOne" class="d-block position-relative text-uppercase collapsible-link py-2">${
          todo.title
        }</a>
                                <span class="float-right mr-6">Due : ${
                                  todo.due_date
                                }</span>
          ...
          todo.status ? "btn-warning text-white" : "btn-success"
        } updateStatusTodo" title="Mark As Done"><i class="fa ${
          todo.status ? "fa-undo" : "fa-check"
        }"></i></button>
                                <button type="button" data-id="${
                                  todo.id
                                }" class="btn btn-sm btn-primary edit-todo"><i class="fa fa-edit"></i></button>
...
$(document).on("submit", "#addProject", function (e) {
  ...
  $.ajax({
    ...
    .fail((err) => {
      toastr.error(
        "Create project unsuccessfull : " + err.responseJSON.msg,
        "Error"
      );
    });
});
